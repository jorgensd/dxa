try:
    ...
    raise ModuleNotFoundError(
        "Moola is not installed. Please install it with `python3 -m pip install git+https://github.com/funsim/moola.git`."
    )
...
class DolfinxVector(Vector):
    ...
    def __init__(self, data: dolfinx.la.Vector, inner_product="L2", riesz_map=None):
        ''' Wraps the Dolfinx object `data` in a moola.DolfinVector object. '''
    ...
    def local_size(self):
        ...
        return self.data.index_map.size_local* self.data.block_size

    def size(self):
        ''' Returns the (gobal) size of the vector. '''
        return self.data.index_map.size_global* self.data.block_size

    def copy(self):
        data_copy = dolfinx.la.vector(self.data.index_map, self.data.block_size)
        data_copy.array[:] = self.data.array[:].copy()
        return self.__class__(data_copy,
